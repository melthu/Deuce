...
from src.pipeline.feature_engineering import _parse_score, order_by_round
...
# What production actually rates with today. Read from the module rather than
# copied, so a re-fit always benchmarks against the constants in force instead
# of the hand-set ones this script originally replaced (those are HAND_SET,
# kept so the original finding stays reproducible). Also seeds the search, so
# a run can only report an improvement on what is already shipped.
SHIPPED = dict(k_base=elo_model.K_BASE, tier_alpha=elo_model.TIER_ALPHA,
               scale=elo_model.SCALE, mov=elo_model.MOV,
               provisional_k=elo_model.PROVISIONAL_K,
               provisional_n=elo_model.PROVISIONAL_N,
               decay=elo_model.DECAY, start=elo_model.START)

HAND_SET = dict(k_base=28.0, tier_alpha=1.0, scale=400.0, mov=0.0,
...
def run_elo(df, k_base=28.0, tier_alpha=1.0, scale=400.0, mov=0.0,
    ...
    for i in range(len(df)):
        ...
        if not skip_arr[i]:
            ...
            mult = 1.0
            if mov:
                parsed = _parse_score(score_arr[i], a_won)
                if parsed:
...
def load_raw(path: str = RAW_PATH) -> pd.DataFrame:
    ...
    for col, default in [("score", ""), ("is_pending", 0), ("is_walkover", 0)]:
        if col not in df.columns:
            df[col] = default
    # Must match the pipeline's ordering exactly, or the constants get fitted
    # against a rating that never compounds within a tournament and then ship
    # into one that does. The original fit used a plain chronological sort,
    # which left every semi-final and final scanned ahead of the rounds that
    # produced them - see order_by_round.
    return order_by_round(df)
